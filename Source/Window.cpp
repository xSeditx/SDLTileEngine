...
void Window::FILLED_BOX(int X1, int Y1, int X2, int Y2) {
	...
	if (X1 < X2) { xt = X1; xb = X2; }
	else { xt = X2; xb = X1; }
	if (Y1 < Y2) { yt = Y1; yb = Y2; }
	else { yt = Y2; yb = Y1; }
...
//  	return NULL;
//  }
//  Widgit *Widgit::MakeFrame(Vec2 pos, Vec2 size) {
//  	return NULL;
//  }
...
////          //SDL_RenderCopy(Renderer, BackBuffer, NULL, NULL);
////  //#if _GETFPS_
////   //   FontRender::Fonts->Write(SCREEN->FPS,Vec2(SCREENWIDTH - 70, 30));
////  //#endif
////          SDL_RenderPresent(Renderer);
////  }
////  
...
////  
////          if (Alive == false) return false;
////          SDL_Point Delta = {(int)Mouse.Position.X - (int)OldMouseX,
////                             (int)Mouse.Position.Y - (int)OldMouseY};
////          OldMouseX = Mouse.Position.X;
////          OldMouseY = Mouse.Position.Y;
////          Mouse.Angle = FindAngle( Mouse.MousePosition, Delta);
////  
